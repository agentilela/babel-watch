...
program.option('-p, --plugins [string]', '', babel.list);
program.option('-b, --presets [string]', '', babel.list);
...
if (program.only != null) only = babel.arrayify(program.only, babel.regexify);
if (program.ignore != null) ignore = babel.arrayify(program.ignore, babel.regexify);

let transpileExtensions = babel.DEFAULT_EXTENSIONS;

if (program.extensions) {
  transpileExtensions = transpileExtensions.concat(babel.arrayify(program.extensions));
}
...
function shouldIgnore(filename) {
  if (transpileExtensions.indexOf(path.extname(filename)) < 0) {
    ...
    return babel.shouldIgnore(filename, ignore || [], only);
  }
}
